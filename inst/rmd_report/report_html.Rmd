...
t %>%
  ...
  kableExtra::footnote(general = glue("<span class = 'footnote'>{foot}</span>"), 
                       escape = FALSE)
```
...
if(any(str_detect(names(t), "\\*"))) {
  tbl <- tbl %>%
    footnote(general = glue("<span class = 'footnote'>&bull; * {note_values}</span>"), 
             escape = FALSE)
}
...
details %>%
  ...
  kable_styling(
    ...
    bootstrap_options = "hover", full_width = FALSE, 
    htmltable_class = "bcgwl-table lightable-classic lightable-hover") %>%
  footnote(general = glue("<span class = 'footnote'>{foot}</span>"), 
           escape = FALSE) %>%
