...
knitr::opts_chunk$set(echo = FALSE, cache.dir = params$cache.dir)
...
# Get Water Year
wy <- unique(w_dates$WaterYear[w_dates$CurrentYear])
...
## Historical Water Level Plots {#plots} 
...
```{r plots, results = "asis", fig.asp = 0.9, fig.width = 10, cache = params$cache_report, cache.extra = w_full_all}
latest_date <- w_dates %>% 
  group_by(ow) %>% 
  filter(!is.na(Value)) %>%
  ...
  for(ow in w_meta$ow[w_meta$region == r]) {
    ...
    w_meta %>%
      ...
      glue_data("\n\n\n#### {ow}\n\n",  # Map title e.g., OW008
                ...
                "**{type} aquifer** | ",
                "**{stringr::str_to_sentence(hydraulic_connectivity)} ",
    ...
    g2 <- well_plot_hist(full_all, hist)
...
w_dates %>%
  ...
  cols_label("area" = "Area", "location" = "Location Name", 
             "ow" = "Obs. Well") %>%
  tab_spanner(label = "Report Datesvs. Measurement Dates", 
