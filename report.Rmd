...
w_full %>%
  ...
  select(district_name, area_name, ow, Date, Value) %>%
  arrange(district_name, area_name, ow, desc(Date)) %>%
  pivot_wider(names_from = "Date", values_from = "Value") %>%
  rename(Region = district_name, `Geographic Area` = area_name, `Obs. Well` = ow) %>%
  kable(booktabs = TRUE, longtable = TRUE, format = "latex", digits = 2, linesep = "",
        escape = FALSE,
        caption = glue("\\textbf{{Well numbers and dates included in this report}}\\\\",
...
t <- well_hist_compare(w_full, report_dates) %>%
  ...
  mutate(class = map_chr(percentile, perc_match, cols = "nice"),
         ...
         district_name = str_remove(district_name, "Natural Resource District"), 
         Value = round(Value, 2), 
         Value = if_else(Approval == "Working", glue("{Value}*"), glue(Value)),
