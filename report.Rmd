...
w_dates %>%
  ...
  mutate(ow = ow_link(ow),
         ...
         Date = if_else(is.na(Value), as_date(NA), Date)) %>%
  arrange(area_name, district_name, ow, desc(report_dates)) %>%
  select(area_name, district_name, ow, report_dates, Date) %>%
  pivot_wider(names_from = "report_dates", values_from = "Date") %>%
  rename(Region = area_name, `Geographic\nArea` = district_name, `Obs.\nWell` = ow) %>%
  kable(booktabs = TRUE, longtable = TRUE, format = "latex", digits = 2, linesep = "",
        escape = FALSE,
        caption = glue("\\textbf{{Well numbers and exact reporting dates}}\\\\",
